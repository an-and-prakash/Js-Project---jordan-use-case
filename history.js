...
function renderHistoryTable() {
    ...
    filteredHistory.forEach(entry => {
        ...
        const typeClass = entry.reportType === "Single Trainer" ? "single-trainer" : "multiple-trainers";
        const trainerNames = entry.reports.map(r => r.trainerName).join(", ");
        const trainerCount = entry.reports.length;
        const traineeCount = entry.fileInfo?.traineeCount || entry.reports[0]?.traineeCount || 0;
        ...
        row.innerHTML = `
            ...
            <td>
                <div class="action-buttons">
                    <button class="action-btn view-btn" onclick="viewReport('${entry.id}')">
                        <i class="fa-solid fa-eye"></i> View
                    </button>
                    <button class="action-btn delete-btn" onclick="deleteReport('${entry.id}', '${entry.reportName}')">
...
window.filterHistory = function() {
    ...
    filteredHistory = allHistory.filter(entry => {
        const matchesSearch = entry.reportName.toLowerCase().includes(searchTerm) ||
                             entry.reports.some(r => r.trainerName.toLowerCase().includes(searchTerm));
