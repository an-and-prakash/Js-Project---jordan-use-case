...
import { getDatabase, ref, get, set, push, update, child }
...
function analyzeFileStructure(headers) {
    ...
    const excludeKeywords = [
        'last modified', 'what went', 'what need', 'attention',
        'completion', 'start', 'id', 'email', 'timestamp', 'response'
    ];

    headers.forEach(header => {
        const headerLower = header.toLowerCase();
        const shouldExclude = excludeKeywords.some(keyword => headerLower.includes(keyword));
        ...
        if (!shouldExclude) {
            ...
            const isRatingColumn = headerLower.includes('rating') ||
    ...
    const trainerNames = Object.keys(trainerGroups).filter(name =>
        trainerGroups[name].length >= 2 
    );
...
function displayFileAnalysis(analysis, columnCount, rowCount) {
    ...
    let infoHTML = `
        ...
        <p><strong>File Type:</strong> ${analysis.type === 'multiple_trainers' ? 'Multiple Trainers' : 'Single Trainer'}</p>
      `;
    ...
    showMessage("success", "Excel file loaded successfully! Please review the analysis above and generate reports.");
}
...
function copyEmailsToClipboard(trainerName) {
    ...
    const emailList = students.map(student => `${student.name} <${student.email}>`).join('\n');
...
async function generateMultipleTrainerReports() {
    ...
    Object.entries(trainerGroups).forEach(([trainer, cols]) => {
        ...
        console.log("ssssssssssssssssssssssssssssssssssssssss", cols)
...
function generateReport(trainerName, columns, commentsWell, commentsImprove) {
    ...
    columns.forEach((c, i) => ratings[i] = { "Excellent": 0, "Very Good": 0, "Good": 0, "Average": 0, "Very Poor": 0 });
  ...
  <div class="report-content">
    ...
    <div class="meta">
      ...
      ${studentsWithVeryPoor.length > 0 ? `<div><strong>Students with "Very Poor" ratings:</strong> ${studentsWithVeryPoor.length}</div>` : ''}
    </div>
    <table>
      <tr>
        <th>Category</th>
    ...
    <div class="section">
      ...
      <ul>
        ${studentsWithVeryPoor.map(student => `<li>${student.name} (${student.email})</li>`).join("")}
      </ul>
    </div>
    ` : ''}
  </div>
...
async function downloadPDF(button) {
    ...
    const trainerName = reportDiv.querySelector("h2")?.textContent.replace("ILP - Tech Fundamentals Feedback — ", "").trim() || "TrainerReport";
